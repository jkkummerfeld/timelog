...
//  Tracker-JKK
//
//  Created by Jonathan Kummerfeld on 11/6/20.
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        coreDataManager = PersistentHistoryObserver(target: .app, persistentContainer: container, userDefaults: UserDefaults(suiteName: "group.name.jkk.TimeLog") ?? UserDefaults.standard)
        coreDataManager?.startObserving()

        // Create the SwiftUI view and set the context as the value for the managedObjectContext environment keyPath.
        // Add `@Environment(\.managedObjectContext)` in the views that will need the context.
        let contentView = ContentView().environment(\.managedObjectContext, container.viewContext)
